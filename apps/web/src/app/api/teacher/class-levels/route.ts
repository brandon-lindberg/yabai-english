...
export async function GET(): Promise<NextResponse> {
  ...
  const classLevels = await prisma.teacherClassLevel.findMany({
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  if (existing?.active) {
    return NextResponse.json(
      { error: "A class level with this name already exists." },
      { status: 409 },
    );
  }

  const last = await prisma.teacherClassLevel.findMany({
    where: { teacherId },
  ...
  const classLevel = await prisma.teacherClassLevel.create({
    data: { teacherId, ...data },
  });
